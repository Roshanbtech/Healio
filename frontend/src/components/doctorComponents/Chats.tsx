...
import DoctorVideoCall from "./Video"; 
...
export default function DoctorChat() {
  ...
  // Instead of creating a local socket, use the global one:
  ...
  // Set up global socket event listeners once (do not create new socket)
  useEffect(() => {
    if (!socket) return;
    ...
    // Listen for notifications from users (for new messages)
    socket.on(
      "notification",
      (notification: {
        chatId: string;
    ...
    return () => {
      // Remove listeners (do not disconnect global socket)
      socket.off("connect");
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="relative">
                        <img
                          src={signedUrltoNormalUrl(user.image) || "https://via.placeholder.com/100"}
                          alt={user.name}
          ...
          <div className="flex-1 flex flex-col bg-white">
            {selectedUser ? (
              <>
                  ...
                  {currentMessages.length > 0 ? (
                    currentMessages.map((message) => (
                      ...
                      >
                        ...
                        >
                          {/* Delete icon for doctor's messages */}
                          {!message.deleted && message.sender === "doctor" && (
                            <div
                ...
                {/* Video Call Modal */}
                {showVideoCall && chatId && currentDoctor && selectedUser && (
                  <DoctorVideoCall
                    chatId={chatId}
                    doctorId={currentDoctor.id}
                    userId={selectedUser._id}
