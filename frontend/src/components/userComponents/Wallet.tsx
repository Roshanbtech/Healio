...
interface IUserWalletInfo {
  wallet: IWallet;
}

const UserWallet: React.FC = () => {
  const [walletInfo, setWalletInfo] = useState<IUserWalletInfo | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchWalletInfo = async () => {
      try {
        ...
        const walletData: IUserWalletInfo = { wallet: profile.wallet };
  ...
  return (
    <div className="flex min-h-screen">
      <div className={`transition-all duration-300 ${sidebarCollapsed ? 'w-16' : 'w-64'} bg-gray-800 text-white h-screen fixed top-0 left-0 z-20`}>
              ...
              <>
                <div className="divide-y divide-gray-200">
                  {currentTransactions.map((transaction, index) => (
                    <div 
                      key={index} 
                      className="p-6 hover:bg-green-100 transition-colors duration-300 group"
                    >
