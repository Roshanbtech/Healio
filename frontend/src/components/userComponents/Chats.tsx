...
export default function Chat() {
  ...
  const [showVideoCall, setShowVideoCall] = useState(false);
  const [incomingCall, setIncomingCall] = useState<any>(null);
  // New states for video call
  const [videoCallChatId, setVideoCallChatId] = useState<string | null>(null);
  const [videoCallDoctorId, setVideoCallDoctorId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    socket.on("notification", (notification: { chatId: string; senderType: "user" | "doctor"; senderId: string; message: string }) => {
      if (notification.senderType === "doctor") {
        setDoctors((prevDoctors) =>
          prevDoctors.map((doc) =>
            doc._id === notification.senderId ? { ...doc, hasNewMessage: true } : doc
  ...
  const handleAcceptCall = (callData: any) => {
    setVideoCallChatId(callData.chatId); // Store the chatId from the incoming call
    setVideoCallDoctorId(callData.callerId); // Store the doctorId from the incoming call
    socket?.emit("video:accept", {
      chatId: callData.chatId,
      callerType: callData.callerType,
      callerId: callData.callerId,
          ...
          <div className="flex-1 flex flex-col bg-white">
            {selectedDoctor ? (
              <>
                ...
                {showVideoCall && videoCallChatId && currentUser && videoCallDoctorId && (
                  <UserVideoCall
                    chatId={videoCallChatId}
                    userId={currentUser.id}
                    doctorId={videoCallDoctorId}
                    onClose={() => {
                      setShowVideoCall(false);
                      setVideoCallChatId(null); // Reset after call ends
                      setVideoCallDoctorId(null); // Reset after call ends
                    }}
                  />
                )}
              </>
