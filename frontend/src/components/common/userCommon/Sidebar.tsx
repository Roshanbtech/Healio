...
export const Sidebar: React.FC<SidebarProps> = ({ onCollapse }) => {
  ...
  useEffect(() => {
    // (optional) you can use the activeIndex for additional logic if needed
    const activeItemIndex = navItems.findIndex(item => item.path === location.pathname);
    // eslint-disable-next-line no-unused-expressions
    activeItemIndex >= 0 && (activeItemIndex);
  }, [location.pathname]);
  ...
  return (
    <div 
      className={`fixed top-0 left-0 h-screen bg-white transition-all duration-300 ease-in-out 
        shadow-[0_0_35px_rgba(0,0,0,0.08)] z-50 overflow-hidden
    ...
    >
      ...
      <div className="relative h-full flex flex-col">
        ...
        <div className="relative w-full h-32 bg-gradient-to-br from-red-600 via-red-500 to-red-700 flex items-center justify-center mb-2 overflow-hidden">
          ...
          >
            {isCollapsed ? (
              <ChevronRight size={18} className="text-red-600 transition-transform duration-300" />
            ) : (
              <X size={18} className="text-red-600 transition-transform duration-300" />
            )}
          </button>
        </div>
        ...
        <nav className="flex-1 px-3 py-2 overflow-y-auto scrollbar-hide">
          <ul className="space-y-2">
            {navItems.map((item, index) => {
              ...
              return (
                <li key={item.label} ref={el => itemRefs.current[index] = el} className="relative">
                  <Link
                    to={item.path}
                  ...
                  >
                    <div className={`flex items-center ${isCollapsed ? 'justify-center w-full' : ''}`}>
                      <div className={`flex items-center justify-center ${
                        isActive ? 'text-white' : 'text-red-600 group-hover:text-green-800'
                      }`}>
        ...
        <div className="p-4 mt-auto">
          <div className="h-px w-full bg-gradient-to-r from-transparent via-red-100 to-transparent mb-4"></div>
          <button
          ...
          >
            <div className={`flex items-center ${isCollapsed ? 'justify-center w-full' : ''}`}>
              <LogOut size={20} className="flex-shrink-0 transition-all duration-200" />
              {!isCollapsed && (
