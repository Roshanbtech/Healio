...
const About = () => {
  ...
  // For doctors slider - using API data
  ...
  const renderStarRating = (rating: number) => {
    ...
    for (let i = 1; i <= 5; i++) {
      if (i <= roundedRating) {
        ...
        // Half star (represented by a lower opacity)
  ...
  return (
    <div className="bg-white">
      ...
      <div className="relative overflow-hidden bg-red-600 text-white rounded-b-3xl">
        ...
        <div className="max-w-6xl mx-auto px-4 py-24 relative z-10">
          <div className="max-w-2xl">
            <h1 className="text-5xl md:text-6xl font-bold mb-6">About Healio</h1>
        ...
        <div className="flex flex-col lg:flex-row items-center gap-16">
          <div className="lg:w-1/2 relative">
            <div className="relative z-10 rounded-2xl overflow-hidden shadow-2xl transform transition-transform hover:scale-105 duration-500 group">
              ...
              <div
                className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent flex items-center justify-center group-hover:opacity-100 transition-opacity duration-300 cursor-pointer"
                onClick={toggleVideo}
              >
                <div className="bg-white/25 backdrop-blur-sm p-4 rounded-full hover:bg-white/40 transition-all duration-300 transform hover:scale-110">
          ...
          <div className="lg:w-1/2">
            ...
            <p className="text-gray-700 mb-6 text-lg">
              Healio is founded by Dr. Sarah Peterson & Dr. James Patel, both Board Certified doctors with over 15 years of experience in healthcare. We understand the challenges individuals face when it comes to accessible and trustworthy medical care.
            </p>
            <p className="text-gray-700 mb-8 text-lg">
              Our mission is to provide an in-depth look at healthcare using innovative ideas, keeping patients informed and involved with personalized diagnostics, monitoring, and health education.
            </p>
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-gray-600 max-w-3xl mx-auto mt-6">
              We combine medical expertise with cutting-edge technology to provide you with the best healthcare experience possible.
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {/* Experience */}
            <div className="bg-white rounded-2xl p-8 shadow-xl border border-gray-100 transition-transform duration-300 hover:scale-105 hover:shadow-2xl hover:border-red-200 group">
      ...
      {/* Team Section - Doctor Slider */}
      <div className="bg-gray-50 py-16 sm:py-24 rounded-3xl mx-auto max-w-7xl my-12">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <div className="inline-block bg-red-600 px-8 py-3 rounded-xl mb-6 shadow-lg transform hover:scale-105 transition-transform duration-300">
              <h2 className="text-3xl sm:text-4xl font-bold text-white">Meet Our Experts</h2>
            </div>
          ...
          {loading ? (
            ...
            <div className="relative">
              ...
              <div className="overflow-hidden rounded-2xl shadow-lg">
                ...
                >
                  {doctors.map((doctor) => (
                    <div key={doctor._id} className="w-full flex-shrink-0 p-4">
                      <div className="flex flex-col md:flex-row items-center gap-8 bg-white rounded-2xl shadow-xl border border-gray-100 transition-all duration-300 hover:shadow-2xl p-6 h-full">
                        {/* Doctor Image in Circle */}
                        <div className="md:w-1/3 flex justify-center mb-6 md:mb-0">
                          <div className="relative">
                            <div className="absolute inset-0 rounded-full bg-gradient-to-r from-red-500 to-red-600 blur-md opacity-20 transform -translate-x-1 translate-y-1"></div>
                            <img
                              src={doctor.image || "/api/placeholder/400/300"}
                              alt={doctor.name}
                              className="w-40 h-40 md:w-48 md:h-48 rounded-full border-4 border-white shadow-xl object-cover z-10 relative"
                            />
                          </div>
                        </div>
                        {/* Doctor Details */}
                        <div className="md:w-2/3">
                          <div className="flex flex-col md:flex-row justify-between items-center mb-3">
                            <h3 className="text-2xl md:text-3xl font-bold text-red-600">{doctor.name}</h3>
                          ...
                          {doctor.speciality && (
                            <p className="inline-block bg-green-50 text-green-800 font-medium px-3 py-1 rounded-full mb-4">
                          ...
                          <button className="mt-4 px-6 py-2.5 rounded-full bg-red-600 text-white hover:bg-red-700 font-medium transition-colors duration-300 shadow-md transform hover:scale-105">
                            Book Appointment
                          </button>
                        </div>
                      </div>
              ...
              <button
                ...
                className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-lg transition-transform hover:scale-110"
              >
                <ChevronLeft className="w-6 h-6 text-red-600" />
              </button>
              <button
                onClick={() => setCurrentDoctor((prev) => (prev === doctors.length - 1 ? 0 : prev + 1))}
                className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-lg transition-transform hover:scale-110"
              >
              ...
              <div className="flex justify-center mt-8 gap-2">
                {doctors.map((_, index) => (
                  <button
                    ...
                    className={`w-3 h-3 rounded-full transition-colors duration-300 ${currentDoctor === index ? "bg-red-600" : "bg-gray-300"}`}
            ...
            <div className="flex justify-center mt-8 gap-2">
              {testimonials.map((_, index) => (
                <button
                  ...
                  className={`w-3 h-3 rounded-full transition-colors duration-300 ${currentTestimonial === index ? "bg-red-600" : "bg-gray-300"}`}
