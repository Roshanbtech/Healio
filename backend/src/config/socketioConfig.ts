...
export const initSocket = (server: HttpServer): SocketIOServer => {
  ...
  io.on("connection", (socket) => {
      ...
      try {
        ...
        io.to(`${recipientType}:${recipientId}`).emit("notification", {
          ...
          message: `You have a new message from ${
            data.sender === "user" ? "the user" : "the doctor"
          }`,
        });
      } catch (error) {
    ...
    socket.on("video:call", async (data) => {
      try {
        const { chatId, callerType, callerId, recipientType, recipientId } =
          data;
        console.log(
          `Video call from ${callerType}:${callerId} to ${recipientType}:${recipientId} in chat ${chatId}`
        );
    ...
    socket.on("video:accept", async (data) => {
      try {
        const { chatId, callerType, callerId, recipientType, recipientId } =
          data;
        console.log(
          `Video call accepted from ${callerType}:${callerId} to ${recipientType}:${recipientId} in chat ${chatId}`
        );
    ...
    socket.on("video:reject", async (data) => {
      try {
        const { chatId, callerType, callerId, recipientType, recipientId } =
          data;
        console.log(
          `Video call rejected from ${callerType}:${callerId} to ${recipientType}:${recipientId} in chat ${chatId}`
        );
    ...
    socket.on("video:end", async (data) => {
      try {
        const { chatId, callerType, callerId, recipientType, recipientId } =
          data;
        console.log(
          `Video call ended from ${callerType}:${callerId} to ${recipientType}:${recipientId} in chat ${chatId}`
        );
    ...
    socket.on("video-signal", async (data) => {
      try {
        ...
        console.log(
          `Video signal from ${callerType}:${callerId} to ${recipientType}:${recipientId} in chat ${chatId}`,
          signal
        );
        io.to(`${recipientType}:${recipientId}`).emit("video-signal", {
          chatId,
